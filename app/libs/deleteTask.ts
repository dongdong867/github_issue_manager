...
export const deleteTask = async (accesstoken: TokenData, task: SendTaskData) => {
  ...
  if (task.number) {
    await axios.patch(`https://api.github.com/repos/${process.env.NEXT_PUBLIC_GITHUB_REPOSITORY_OWNER}/${process.env.NEXT_PUBLIC_GITHUB_REPOSITORY}/issues/${task.number}`, {
      state: 'closed'
    })
